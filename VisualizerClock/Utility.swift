...
class Utility
{
    ...
    public static func GetTimeStampToEncode(From: Date) -> String
    {
        var Final = ""
        let AddDate = _Settings.bool(forKey: Setting.Key.IncludeDate)
        let AddWeekday = _Settings.bool(forKey: Setting.Key.IncludeWeekday)
        var Format = DateParts.TimeOnly
        if AddDate
        {
            switch _Settings.integer(forKey: Setting.Key.EncodingOrder)
            {
            case 0:
                Format = DateParts.TimeAndDate
                
            case 1:
                Format = DateParts.DateAndTime
                
            default:
                Format = DateParts.TimeAndDate
            }
        }
        Final = DateToString(From, Parts: Format, IncludeSeconds: _Settings.bool(forKey: Setting.Key.IncludeSeconds),
                             DateIncludesWeekday: AddWeekday, SeparateWithComma: true)
        return Final
    }
    
    /// Return the time stamp as an integer value. The form is in YYYYMMDDWWHHMMSS with each date component
    /// separated by 60x - in other words, each component is in a separate base 60 column.
    ///
    /// - Parameter From: The date to convert to an integer time stamp.
    /// - Parameter Long: If true, a longer format is used.
    /// - Returns: Time stamp of the passed date with optional information based on current user settings.
    public static func GetTimeStampToEncodeI(From: Date, _ Long: Bool = false) -> Int
    {
        var Final = 0
        let AddDate = _Settings.bool(forKey: Setting.Key.IncludeDate)
        let AddWeekday = _Settings.bool(forKey: Setting.Key.IncludeWeekday)
        ...
        if Long
        {
            let TurnOf20thCentury = Date.StartOfTwentiethCentury()
            let TurnOf21stCentury = Date.StartOfTwentyFirstCentury()
            let Days = Date.DaysBetween(TurnOf21stCentury, TurnOf20thCentury)
            var Seconds = Date.SecondsBetween(TurnOf21stCentury, From)
            var DayLength = log10(Double(Days))
            DayLength = Double(Int(DayLength) + 1)
            let Multiplier = pow(10, DayLength)
            Seconds = Seconds * Int(Multiplier)
            Final = Days + Seconds
        }
        else
        {
            let Second = Cal.component(.second, from: From)
            let Minute = Cal.component(.minute, from: From) + 60
            let Hour = Cal.component(.hour, from: From) + (60 * 2)
            let WeekDay = Cal.component(.weekday, from: From) + (60 * 3)
            let Day = Cal.component(.day, from: From) + (60 * 4)
            let Month = Cal.component(.month, from: From) + (60 * 5)
            let Year = Cal.component(.year, from: From) + (60 * 6)
            Final = Second + Minute + Hour
            if AddWeekday
            {
                Final = Final + WeekDay
            }
            if AddDate
            {
                Final = Final + Day + Month + Year
            }
        }
        return Final
    }
    
    /// Describes the use of a color.
    ///
    /// - TextInterior: Used to paint the interior of text for textual time.
    /// - TextOutline: Used to paint the outline of text for textual time.
    /// - Background: Background color.
    /// - GradientTop: First gradient color for gradient backgrounds.
    /// - GradientBottom: Second gradient color for gradient backgrounds.
    ...
    static func GetColorControl(Key: String) -> Int
    {
        return _Settings.integer(forKey: Key)
    }
    
    /// Given an index into dark mode levels, return the actual dark mode brightness multiplier.
    ///
    /// - Parameter Index: Index into dark mode multipliers.
    /// - Returns: Dark mode multiplier that corresponds to the passed index.
    static func VarianceFromIndex(_ Index: Int) -> CGFloat
    {
        switch Index
        {
        case 0:
    ...
    static func DarkModeVariance(_ Index: Int) -> CGFloat
    {
        switch Index
        {
        case 0:
    ...
    /// Depending on the value of InDarkMode, modify the color as per the settings.
    ///
    /// - Parameters:
    ///   - Source: Source color
    ///   - InDarkMode: Determines if a modified color is returned.
    /// - Returns: If dark mode is on, a new color based on user settings, else the same color unchanged.
    public static func ModifyColorForDarkMode(_ Source: UIColor, InDarkMode: Bool) -> UIColor
    {
        if !InDarkMode
        {
            //print("Not in dark mode.")
            return Source
        }
        if !_Settings.bool(forKey: Setting.Key.DarkModeChangeBrightness) && !_Settings.bool(forKey: Setting.Key.DarkModeChangeSaturation)
        {
            print("No dark mode actions specified.")
            return Source
        }
        let (H, S, B) = GetHSB(SourceColor: Source)
        var NewB: CGFloat = B
        var NewS: CGFloat = S
        if _Settings.bool(forKey: Setting.Key.DarkModeChangeBrightness)
        {
            let BrightVariance = DarkModeVariance(_Settings.integer(forKey: Setting.Key.DarkModeRelativeBrightness))
            NewB = B * BrightVariance
        }
        if _Settings.bool(forKey: Setting.Key.DarkModeChangeSaturation)
        {
            let SatVariance = DarkModeVariance(_Settings.integer(forKey: Setting.Key.DarkModeRelativeSaturation))
            NewS = S * SatVariance
        }
        let NewColor = UIColor(hue: H, saturation: NewS, brightness: NewB, alpha: 1.0)
        //print("DarkModeColor: \(PrintHSBColor(Source)) => \(PrintHSBColor(NewColor))")
        return NewColor
    }
    
    /// Finalize a color to use as the foreground color.
    ///
    /// - Parameters:
    ///   - BaseColor: The base color that may be altered
    ///   - InDarkMode: True if we're currently in dark mode, false if not.
    /// - Returns: Potentially altered color to use for foreground objects.
    public static func FinalizeForegroundColor(BaseColor: UIColor, InDarkMode: Bool) -> UIColor
    {
        let ColorDeterminant = GetColorControl(Key: Setting.Key.BarcodeForegroundColorMethod)
        switch ColorDeterminant
        {
        case 0:
            //Variable color based on the background color.
            var HueVariance: CGFloat = 0.0
            if _Settings.bool(forKey: Setting.Key.BarcodeForegroundUseManualVariance)
            {
                HueVariance = CGFloat(_Settings.double(forKey: Setting.Key.BarcodeForegroundColorVariance))
            }
            else
            {
                let Variance = _Settings.integer(forKey: Setting.Key.BarcodeForegroundStandardVariance)
                HueVariance = VarianceFromIndex(Variance)
            }
            let NewColor = MakeHSBColorFrom(BaseColor: BaseColor, WithVariance: HueVariance)
            return ModifyColorForDarkMode(NewColor, InDarkMode: InDarkMode)
            
        case 1:
            //Black
            return ModifyColorForDarkMode(UIColor.black, InDarkMode: InDarkMode)
            
        case 2:
            //White
            return ModifyColorForDarkMode(UIColor.white, InDarkMode: InDarkMode)
            
        case 3:
            //User-specified
            return ModifyColorForDarkMode(BaseColor, InDarkMode: InDarkMode)
            
        default:
            //Unexpected - return black
            return ModifyColorForDarkMode(UIColor.black, InDarkMode: InDarkMode)
        }
    }
    
    /// Perform final modification of a color depending on the context. Context is described by
    /// various parameters and settings.
    ///
    /// - Parameters:
    ///   - Color: The color to potentially change. Whether the color is changed or not depends on the context.
    ///   - BaseColor: The base (usually background) color that will be used in certain circumstances.
    ///   - OfType: Determines the type of color we are modifying.
    /// - Returns: Possibly modified color.
    public static func FinalizeColor(_ Color: UIColor, BaseColor: UIColor, OfType: ColorTypes) -> UIColor
    {
        let DarkModeOn = _Settings.bool(forKey: Setting.Key.InDarkMode)
        switch OfType
        {
        case .Barcode:
            let ColorDeterminant = GetColorControl(Key: Setting.Key.BarcodeForegroundColorMethod)
            ...
            {
            case 0:
                //Variant of BaseColor - ignore Color.
                var FinalVariance: CGFloat = 0.0
                if _Settings.bool(forKey: Setting.Key.BarcodeForegroundUseManualVariance)
                {
                    let Variance = CGFloat(_Settings.double(forKey: Setting.Key.BarcodeForegroundColorVariance))
                    FinalVariance = Variance
                }
                else
                {
                    let Variance = _Settings.integer(forKey: Setting.Key.BarcodeForegroundStandardVariance)
                    FinalVariance = VarianceFromIndex(Variance)
                }
                let NewColor = MakeHSBColorFrom(BaseColor: Color, WithVariance: FinalVariance)
                return ModifyColorForDarkMode(NewColor, InDarkMode: DarkModeOn)
                
            case 1:
                //No change - assumed to be black
                return ModifyColorForDarkMode(UIColor.black, InDarkMode: DarkModeOn)
                
            case 2:
                //No change - assumed to be white
                return ModifyColorForDarkMode(UIColor.white, InDarkMode: DarkModeOn)
        ...
        case .TextInterior:
            let ColorDeterminant = GetColorControl(Key: Setting.Key.TextColor)
            switch ColorDeterminant
            {
            case 0:
                //Variant of BaseColor - ignore Color.
                var FinalVariance: CGFloat = 0.0
                if _Settings.bool(forKey: Setting.Key.TextColorVarianceIsNonStandard)
                {
                    let Variance = CGFloat(_Settings.double(forKey: Setting.Key.ManualTextColorVariance))
                    FinalVariance = Variance
                }
                else
                {
                    let Variance = _Settings.integer(forKey: Setting.Key.TextColorVariance)
                    FinalVariance = VarianceFromIndex(Variance)
                }
                let NewColor = MakeHSBColorFrom(BaseColor: BaseColor, WithVariance: FinalVariance)
                return ModifyColorForDarkMode(NewColor, InDarkMode: DarkModeOn)
                
            case 1:
                //No change - assumed to be black
                return ModifyColorForDarkMode(Color, InDarkMode: DarkModeOn)
        ...
        case .TextOutline:
            let ColorDeterminant = GetColorControl(Key: Setting.Key.OutlineColor)
            switch ColorDeterminant
            {
            case 0:
                //Variant of BaseColor - ignore color.
                let Variance = _Settings.integer(forKey: Setting.Key.OutlineColorVariance)
                let FinalVariance = VarianceFromIndex(Variance)
                let NewColor = MakeHSBColorFrom(BaseColor: BaseColor, WithVariance: FinalVariance)
                return ModifyColorForDarkMode(NewColor, InDarkMode: DarkModeOn)
    ...
    #if false
    /// iDevice models (as of September 2018).
    ///
    /// - iPadPro129: iPad Pro 12.9 inch
    /// - iPadPro105: iPad Pro 10.5 inch
    /// - iPadPro97orMini: iPad Pro 9.7 inch or iPad Mini 4 7.9 inch
    /// - iPad97: iPad 9.7 inch
    /// - iPhoneX: iPhone X
    /// - iPhone: iPhone 8, 7, 6s
    /// - iPhoneSE: iPhone SE
    ...
    /// Map from longest dimension to model type.
    private static let PortraitModelData =
        [
            (2732, Models.iPadPro129),
            (2224, Models.iPadPro105),
            (2048, Models.iPadPro97orMini),
            (2436, Models.iPhoneX),
            (2208, Models.iPhonePlus),
            (1334, Models.iPhone),
            (1136, Models.iPhoneSE),
            (1792, Models.iPhoneXR),
            (2668, Models.iPhoneXSMax)
    ]
    
    /// Attempts to deduce the model the program is running on by checking the longest dimension
    /// of the screen. Note that some models have the same dimensional size.
    /// https://stackoverflow.com/questions/46192280/detect-if-the-device-is-iphone-x
    ///
    /// - Returns: Deduced model type.
    public static func ModelX() -> Models
    {
        let ScreenHeight = UIScreen.main.nativeBounds.height
        let ScreenWidth = UIScreen.main.nativeBounds.width
        let Longest = max(ScreenWidth, ScreenHeight)
        print("Longest dimension is \(Int(Longest))")
        for (Height, ForModel) in PortraitModelData
        {
            if Height == Int(Longest)
            {
                return ForModel
            }
        }
    ...
    ///
    /// - LeftButton: The left menu button.
    /// - RightButton: The right menu button.
    ...
    public static func GetPosition(ForElement: UIElements, Element: UIView, Width: Int, Height: Int) -> CGSize?
    {
        return nil
    }
    
    public static func HidingPosition(ForElement: UIElements, Element: UIView, Width: Int, Height: Int) -> CGSize?
    {
        return nil
    }
    
    /// Given a screen resolution, return the locations of where UI elements should appear.
    ///
    /// - Parameters:
    ///   - For: The UI element whose offset will be returned.
    ///   - ScreenWidth: Screen width (in pixels).
    ///   - ScreenHeight: Screen height (in pixels).
    ///   - ForWhenVisible: Determines if the offset will be for on the screen (visible) or off the screen (invisible).
    /// - Returns: Point with an offset to apply to the UI element to move it to the desired location.
    public static func CalculateElementOffset(For: UIElements, ScreenWidth: Int, ScreenHeight: Int,
                                              ForWhenVisible: Bool = true) -> CGPoint?
    {
        let Resolution = (ScreenWidth, ScreenHeight)
        switch Resolution
        {
        case (640,1136):
            //iPhone 5s, SE in portrait mode
            break
        ...
        case (750, 1334):
            //iPhone 6 - 8{s} in portrait mode
            break
            
        case (1334, 750):
            //iPhone 6 - 8{s} in landscape mode
            break
            
        case (1125, 2436):
            //iPhone X{s} in portrait mode
            break
            
        case (2436, 1125):
            //iPhone X{s} in landscape mode
            break
        ...
        case (1242,2688):
            //iPhone XS Max in portrait mode
            break
            
        case (2688, 1242):
            //iPhone XS Max in landscape mode
            break
            
        case (1536,2048):
            //iPad minis through iPad Pro 9.7 inch in portrait mode
            break
            
        case (2048,1536):
            //iPad minis through iPad Pro 9.7 inch in landscape mode
            break
            
        case (1668,2224):
            //iPad Pro 10.5 inch in portrait mode
            break
            
        case (2224,1668):
            //iPad Pro 10.5 inch in landscape mode
            break
            
        case (2048,2732):
            //iPad Pro 12.9 inch generations in portrait mode
            break
            
        case (2732,2048):
            //iPad Pro 12.9 inch generations in landscape mode
            break
            
        default:
            print("Unknown resolution encountered: \(Resolution)")
            return nil
        }
        return nil
    }
